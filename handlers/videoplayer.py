    ...
    if not replied:
        if len(m.command) < 2:
            ...
            try:
                ...
                return
            if not livelink:
                await m.reply("video verisi alınamadı")
                return
...
@Client.on_message(command(["vbitir", f"vbitir@{Veez.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
